    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Global Quick Search" maxWidth="lg">
      <div className="space-y-4">
        ...
        <div className="max-h-72 overflow-y-auto space-y-4 pr-1">
          {/* Clients Section */}
          <div>
            <h4 className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Clients</h4>
            <div className="space-y-1">
              {clientsData?.data.slice(0, 4).map((c: any) => (
                <div
                  key={c.id}
                  onClick={() => handleSelect(`/clients/${c.id}`)}
                  className="flex items-center justify-between p-2 rounded-lg hover:bg-slate-50 cursor-pointer transition-smooth group"
                >
                  <div className="flex items-center space-x-3">
                    <User className="w-4 h-4 text-blue-600" />
                    <div>
                      <p className="text-xs font-semibold text-slate-900">{c.name}</p>
                      <p className="text-[10px] text-slate-500">PAN: {c.pan} | GSTIN: {c.gstin || "N/A"}</p>
                    </div>
                  </div>
                  <ArrowRight className="w-4 h-4 text-slate-400 group-hover:text-blue-600 transition-smooth" />
                </div>
              ))}
            </div>
          </div>
          ...
          {/* ITR Filings Section */}
          <div>
            <h4 className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">ITR Filings</h4>
            <div className="space-y-1">
              {itrData?.data.slice(0, 3).map((itr: any) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-slate-900">{itr.clientName} — {itr.assessmentYear}</p>
                      <p className="text-[10px] text-slate-500">Form: {itr.itrFormType} | Status: {itr.status}</p>
                    </div>
                  </div>
                  <ArrowRight className="w-4 h-4 text-slate-400 group-hover:text-emerald-600 transition-smooth" />
                </div>
              ))}
            </div>
          </div>
