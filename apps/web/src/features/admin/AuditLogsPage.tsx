...
export const AuditLogsPage: React.FC = () => {
  ...
  const columns: Column<AuditLogRecord>[] = [
    {
      header: "Timestamp",
      cell: (row) => <span className="font-mono text-[11px] text-slate-600">{new Date(row.createdAt).toLocaleString()}</span>
    },
    {
      header: "User / Actor",
      cell: (row) => (
        <span className="text-xs font-semibold text-slate-900">{row.userName}</span>
      )
    },
    {
    ...
    },
    {
      ...
      cell: (row) => <span className="text-xs font-medium text-slate-700">{row.entityType} ({row.entityId.substring(0, 8)}...)</span>
    },
    {
      header: "Payload Diff",
      cell: (row) => (
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-900 tracking-tight">Practice Audit Log Stream</h1>
        <p className="text-xs text-slate-500 mt-1">Immutable security log recording all client creation, status transitions, uploads, and billing modifications.</p>
      </div>
