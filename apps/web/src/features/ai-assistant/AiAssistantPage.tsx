    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Hybrid Intent Engine querying live practice database and local Ollama vectors.</p>
        </div>
      </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
            {messages.map((m) => (
              ...
              >
                ...
                >
                  {m.sourceType && (
                    <div className="flex items-center space-x-1 text-[9px] font-bold uppercase tracking-wider text-slate-400 mb-1.5 border-b border-slate-100 pb-1">
                ...
                {m.role === "user" && (
                  <div className="w-7 h-7 rounded-full bg-slate-800 text-white flex items-center justify-center font-bold text-xs flex-shrink-0 mt-1">
                    U
                  </div>
                )}
              </div>
          ...
          <div className="px-4 py-2 bg-white border-t border-slate-100 flex items-center gap-2 overflow-x-auto text-[11px]">
            ...
            {quickPrompts.map((p, i) => (
              <button
                ...
                className="px-2.5 py-1 rounded-full bg-slate-100 hover:bg-blue-50 hover:text-blue-700 text-slate-600 font-medium whitespace-nowrap transition-smooth border border-slate-200"
              >
                {p}
              </button>
            ))}
        ...
        <Card className="h-full space-y-4">
          ...
          <div className="space-y-3 text-xs">
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <p className="font-bold text-slate-900 mt-0.5">Ollama llama3.1:8b</p>
            </div>

            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200">
              <span className="text-slate-500 font-medium">Vector Embeddings</span>
              <p className="font-bold text-slate-900 mt-0.5">nomic-embed-text (pgvector 768d)</p>
            </div>

            <div className="p-3 bg-blue-50/50 rounded-lg border border-blue-100 text-blue-900">
              <p className="font-semibold text-[11px]">🔒 Permission-Scoped Retrieval</p>
              <p className="text-[10px] text-blue-700 mt-1">Queries strictly enforce staff row-level access rules before returning context.</p>
            </div>
          </div>
        </Card>
      </div>
