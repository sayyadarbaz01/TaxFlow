  ...
  const columns: Column<WhatsAppMessageRecord>[] = [
    {
      ...
      cell: (row) => (
        <div>
          ...
          <span className="text-[10px] text-slate-500">{new Date(row.sentAt).toLocaleString()}</span>
        </div>
      )
    },
    {
      ...
      cell: (row) => (
        <span
          className={`px-2 py-0.5 rounded text-[10px] font-bold ${
            row.direction === "INBOUND" ? "bg-blue-50 text-blue-700" : "bg-emerald-50 text-emerald-700"
          }`}
        >
          {row.direction}
        </span>
    ...
    {
      header: "Message Body",
      cell: (row) => <p className="text-xs text-slate-800 max-w-xs truncate">{row.body}</p>
    },
    {
      header: "Statutory Cost",
      cell: (row) => <span className="font-mono text-xs font-semibold text-slate-700">₹{row.cost.toFixed(2)}</span>
    },
    {
      header: "Status",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">WhatsApp Communication Center</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-200">
          ...
          <p className="text-2xl font-bold text-slate-900">₹{spendData?.totalSpend ? spendData.totalSpend.toFixed(2) : "0.00"}</p>
          <p className="text-[10px] text-slate-500 mt-1">{spendData?.month || "Current"} Billing Cycle</p>
        </Card>

        <Card>
          <span className="text-xs font-semibold text-slate-500">Outbound Messages Sent</span>
          <p className="text-2xl font-bold text-slate-900 mt-2">{spendData?.totalSent || 0}</p>
        </Card>

        <Card>
          <span className="text-xs font-semibold text-slate-500">Approved Templates</span>
          <p className="text-2xl font-bold text-blue-600 mt-2">{templatesData?.length || 2}</p>
        </Card>
      </div>

      {/* Quick Dispatch Section */}
      <Card>
        <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider mb-3">Quick Dispatch WhatsApp Reminder</h3>
        {clients.length === 0 ? (
          ...
          <form onSubmit={handleSendSubmit} className="flex flex-wrap items-center gap-3">
            ...
            <select
              ...
              className="text-xs border border-slate-300 rounded-lg px-3 py-2 bg-white"
            >
              <option value="compliance_due_reminder">Template: Compliance Due Reminder</option>
              <option value="doc_request_reminder">Template: Document Request Reminder</option>
            </select>
      ...
      {/* Message Logs Table */}
      <Table columns={columns} data={messagesData?.data || []} isLoading={isLoading} emptyText="No WhatsApp message logs recorded." />
    </div>
  );
};
